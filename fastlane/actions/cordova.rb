  ...
  module Actions
    class CordovaAction < Action
      def self.run(params)
        ...
        sh "pwd && . $NVM_DIR/nvm.sh && nvm use #{node_version} && cd #{Actions.lane_context[SharedValues::CORDOVA_PATH]} && export PARTH=$PATH:node_modules/.bin && cordova #{params[:action]} #{params[:platform]} &&  && pwd"
      end
